...
def entry_price_select_html(entry_price: str = 'free') -> str:
    ...
    name = 'free_entry'
    options = ['<option value="free">Free Entry</option>',
               '<option value="paid">Pay-for-Entry</option>']
    if entry_price == 'paid':
...
def version_select_html(version: str = 'openrct') -> str:
    ...
    name = 'version'
    options = ['<option value="openrct">OpenRCT2</option>',
...
def ages_select_html(age_selection: str = 'All') -> str:
    ...
    options = [
        f'<option value="{age}">{age}</option>' for age in new_age_options]
    return select_html(text, name, options)
...
def process_request_for_calculation(request_dot_method):
    ridenames = tuple(
        rn.name for rn in RideName.objects.all() if rn.is_visible)
    # set defaults
    context = {
        'ridenames': ridenames,
        'ride_name': '',
    ...
    ride_name = request_dot_method.get('ridename')
    keys = ['excitement_rating', 'intensity_rating', 'nausea_rating']
    EIN_str = [request_dot_method.get(key) for key in keys]
    EIN = get_EIN(EIN_str)
    ...
    version = request_dot_method.get('version')
    context['version_select_html'] = version_select_html(version)
    ages = request_dot_method.get('ages')
...
def calculator_view(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        # print(f'POST-request: {request.POST}')
        context = process_request_for_calculation(request.POST)
    if request.method == 'GET':
        # print(f'GET-request{request.GET}')
        context = process_request_for_calculation(request.GET)
